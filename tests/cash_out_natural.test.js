...
const mockUsers = [
    ...
    //new week
    { "date": "2016-01-18", "user_id": 1, "user_type": "natural", "type": "cash_out", "operation": { "amount": 600.00, "currency": "EUR" } },
    { "date": "2016-01-19", "user_id": 1, "user_type": "natural", "type": "cash_out", "operation": { "amount": 600.00, "currency": "EUR" } }
];
...
describe("Natural type user's commissions", () => {
    ...
    //new week
    test('If new week and money <= 1000EUR - free of charge.', () => {
        expect(cash_out_natural.cashOutNatural(mockUsers[3], config)).toEqual(0.00);
    });
    test('If week\'s money > 1000EUR and week\'s money minus 1000EUR < 1000EUR - fee calculating (week\'s money - 1000EUR)', () => {
        expect(cash_out_natural.cashOutNatural(mockUsers[4], config)).toEqual(0.60);
    });
});